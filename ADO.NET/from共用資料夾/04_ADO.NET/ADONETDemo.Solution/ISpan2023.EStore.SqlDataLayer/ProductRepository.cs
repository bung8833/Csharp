...
(@CategoryId , @Name, @Description, @Price, @Status , @ProductImage, @Stock)";
			...
			var parameters = SqlParameterBuilder.Create()
				...
				.AddNVarchar("ProductImage", 70, entity.ProductImage)
...
INNER JOIN Categories as C ON P.CategoryId = C.Id";
			...
			//{
			//	where += $" AND P.CategoryId={categoryId.Value}"; // 最前面要加空白
			//}

			//if (string.IsNullOrEmpty(productName) == false)
			//{
			//	where += $" AND P.Name LIKE '%' + @ProductName + '%'";
			//	parameters.Add(new SqlParameter("ProductName", SqlDbType.NVarChar, 50) { Value = productName });
			//}

			//sql = string.IsNullOrEmpty(where) ? sql : sql + " WHERE " + where.Substring(5);
			...
			//		while (reader.Read())
			//		{
			//			ProductDto product = ProductDto.GetInstance(reader);
	...
	public class ProductDto
	{
		...
		public static ProductDto GetInstance(SqlDataReader reader)
		{
			...
			dto.Id = reader.GetInt32("Id", 0);
			dto.CategoryName = reader.GetString("CategoryName");
			dto.ProductName = reader.GetString("Name");
			dto.Price = reader.GetInt32("Price", 0);
