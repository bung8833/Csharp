    ...
    public partial class Form1 : Form
    {
        ...
        private void buttonGetConnection_Click(object sender, EventArgs e)
        {
            ...
            using ( SqlConnection conn = SqlDb.GetConnection("default") ) {
                using ( var commnd = new SqlCommand("SELECT * FROM Categories", conn) ) {
                    conn.Open();
		...
		{
            bool isInt = int.TryParse(txtCategoryId.Text, out int value);
		...
		private void buttonDeleteNews_Click(object sender, EventArgs e)
		{
			...
			if (!isInt)
			{
				MessageBox.Show("請輸入正確 News Id，再試一次");
				return;
			}
			...
			{
				MessageBox.Show("紀錄沒有被刪除，可能紀錄不存在");
			}
		}

		private void buttonMaintianUser_Click(object sender, EventArgs e)
		{
