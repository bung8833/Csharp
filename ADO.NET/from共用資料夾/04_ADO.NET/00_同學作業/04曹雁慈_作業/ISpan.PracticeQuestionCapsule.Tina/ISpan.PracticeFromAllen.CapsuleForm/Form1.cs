...
namespace ISpan.PracticeFromAllan.CapsuleForm
{
	public partial class Form1 : Form
	{
		...
		{
			isNumber = int.TryParse(textBoxCoin.Text, out int coin);
			if (isNumber == false) return; // 用衛述句,讓程式碼不要在太多層的if裡
			
			wallet = CapsuleUtility.ClacCoin(wallet, coin);
			resultstring();
			buttonEnable();
		...
		{
			if (CapsuleUtility.CanRefund(wallet) == false) return;
		...
		{
			if (CapsuleUtility.CanShip(wallet))
			{
				wallet = 0;
		...
		{
			switch (CapsuleUtility.CapsuleState(wallet))
			{
				case 0:
		...
		{
			this.buttonRefund.Enabled = CapsuleUtility.CanRefund(wallet);
			this.buttonShip.Enabled = CapsuleUtility.CanShip(wallet);
		}
